...
using financialApp.Services;  // Dodaj tę linię, aby zaimportować FinancialService
...
namespace financialApp.Views
{
    public partial class UserDashboard : Window, INotifyPropertyChanged
    {
        ...
        public UserDashboard(IUserService userService, FinancialService financialService, int userId)
        {
            InitializeComponent();
            DataContext = this;
        ...
        private void GenerateFinancialReport_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                DateTime startDate = new DateTime(2023, 1, 1);
                DateTime endDate = DateTime.Today;
                var report = _financialService.GetFinancialReport(_userId, startDate, endDate);
                string reportMessage = "Financial Report:\n";
                foreach (var item in report)
                {
                    reportMessage += $"UserID: {item.UserID}, Total Incomes: {item.TotalIncomes}, Total Spendings: {item.TotalSpendings}, Balance: {item.Balance}\n";
                }
                MessageBox.Show(reportMessage, "Financial Report", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error generating financial report: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }

        private void AverageMonthlySpendings_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var averageSpendings = _financialService.GetAverageMonthlySpendings(_userId);
                MessageBox.Show($"Average Monthly Spendings: {averageSpendings:F2}", "Average Monthly Spendings", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error calculating average monthly spendings: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }

        private void FinancialReportFunction_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                DateTime startDate = new DateTime(2023, 1, 1);
                DateTime endDate = DateTime.Today;
                var report = _financialService.GetFinancialReport(_userId, startDate, endDate);
                string reportMessage = "Financial Report Function:\n";
                foreach (var item in report)
                {
                    reportMessage += $"UserID: {item.UserID}, Total Incomes: {item.TotalIncomes}, Total Spendings: {item.TotalSpendings}, Balance: {item.Balance}\n";
                }
                MessageBox.Show(reportMessage, "Financial Report Function", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error generating financial report: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
